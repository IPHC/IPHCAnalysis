...
{
  double sum_pt_min = -999999;
  double sum_pt = -999999;
  double M3_ = 500;
  
  for(unsigned int i=0; i<inputVectors_.size(); ++i){
    for(unsigned int j=i+1; j<inputVectors_.size(); ++j){
      for(unsigned int s=j+1; s<inputVectors_.size(); ++s){
	
	TLorentzVector p4 = inputJets[i].p4 + inputJets[j].p4 + inputJets[s].p4;	
	sum_pt =p4.Et();
	if(sum_pt > sum_pt_min) sum_pt_min = sum_pt;
