	...
	for(unsigned int varNumber =0; varNumber < Histos.size();varNumber++){
		...
		for(unsigned int iChannel = 0;iChannel<Channels.size();iChannel++){
			...
			//And cut by cut histos	
                        ...
                        if (check1 && check2) {
                         dir2 = dir1->GetDirectory("CutByCut");
			 if(!dir2) dir2 = dir1->mkdir("CutByCut");
			 dir2->cd();
			...
			for(unsigned int iSelStep=0;iSelStep<SelectionSteps.size();iSelStep++){
				...
				if(SumDataDatasetsHistos.size()>0) SumDataDatasetsHistos[varNumber][iSelStep][iChannel].Write();
...
void HistoManager::SetCutFlowAxisTitleFCNCMonotop(vector<TH1F>& hlist, string v, string datasetname){

  for (unsigned int i=0; i < hlist.size(); i++){
  
     hlist[i].GetXaxis()->SetLabelSize(0.07);
     hlist[i].GetXaxis()->SetBinLabel(1, "trigger");
     hlist[i].GetXaxis()->SetBinLabel(2, "lept. sel.");
