...
void GeneralMMEstimation::CountNSel(const SSDiLeptonSelection & sel, Dataset dataset, string channelName, float weight, string CandTypeRequested, int selStepCut, const NTMonteCarlo* mc){

   int selStepMM[4]= {-1, -1, -1, -1};
   string CandTypeForMM[4]= {"", "", "", ""};
   unsigned int njets[4] = {-1, -1, -1, -1};
   bool passSelection[4] = {false, false, false, false};
   ...
   for(unsigned int bin_index = 0; bin_index < theNBins; bin_index++){
     for(unsigned int iso_index = 0; iso_index < 4; iso_index++){
       ...
       if((CandTypeForMM[iso_index] == CandTypeRequested) && (selStepMM[iso_index] >= selStepCut)  && ((njets[iso_index] >= (((theMaxBin-theMinBin)/theNBins)*bin_index)) && (njets[iso_index] < (((theMaxBin-theMinBin)/theNBins)*(bin_index+1))))){
         IncrementNSelBin(iso_index, bin_index, weight);
	 passSelection[iso_index] = true;
       }
     }

     if( passSelection[3] && (!passSelection[2] || !passSelection[1] || !passSelection[0]) )
     std::cout << "WARNING in CountNSel: Passing Selection TT, but not passing selection TL or LT or LL!" << std::endl;

     if( passSelection[2] && !passSelection[0])
   ...
   if(mc->partonFlavor.first!=0 && mc->partonFlavor.second!=0){ // Only for MC. Partons set to 0 for data
	 ...
	 if((mc->TMEME == 11 || mc->TMEME == 11001 || mc->TMEME == 10110 || mc->TMEME == 21100) && dataset.Name() == "TTbar" // emu
	 ){
           FillMMExpectedPlot("TTbarSignal",iso_index,njets[iso_index],weight);
	 }else if((mc->TMEME == 1 || mc->TMEME == 10 || // l(=e/mu)jets
                  mc->TMEME == 2 || mc->TMEME == 10101 || mc->TMEME == 20200 || // ee
		  mc->TMEME == 20 || mc->TMEME == 11010 || mc->TMEME == 22000 || // mumu                
                  mc->TMEME == 10010 || mc->TMEME == 10001 || // l(=e/mu)tau(->had)
	          mc->TMEME == 10100 || mc->TMEME == 11000) && dataset.Name() == "TTbar" // tau(->e/mu)jets
	 ){
