...
void Efficiency_MC_Estimation ( string xmlFileName, bool mc=true, string lepton="Mu")
{
  ...
  // isolations cuts
  float looseIso = 0.8 ; //0.8; 999
  float tightIsoMMmu = 0.2; float tightIsoMMe = 0.17;
	...
	NJets->Fill(jets_Tight.size());
	 ...
	 if(leptons.size() == 1){
	    ...
	    //          if(mT < 80 && mT > 65) ainvmasscutW = false;
          ...
          if(leptons.size() == 1 && met.p2.Mod() < 20 && jets.size() >= 1 && ainvmasscutW && datasets[d].Name() == "DataJet"){   	    
...
int main ()
{
  ...
  Efficiency_MC_Estimation("../../config/MatrixMethod_MC.xml", true, "E");  
  //Efficiency_MC_Estimation("../../config/MatrixMethod_Data.xml", false, "Mu");
  //Efficiency_MC_Estimation("../../config/MatrixMethod_Data.xml", false, "E");  
