...
class DiLeptonSelection: public Selection{
   ...
   public:
      ...
      bool GetLeptonPair(std::vector<NTMuon> muon_in,std::vector<IPHCTree::NTElectron> elec_in, std::vector<NTMuon>& muon_out,std::vector<NTElectron>& elec_out,  string& CandPairType, bool isForMM = false, float iso1_in = -1., float iso2_in = -1.);
...
//      bool passTriggerSelection(string datasetname, string channelName = string(""));
	...
	*/
    ...
    private:
      ...
      SFBweight sfb_dilep_; //JLA Temporary solution to use getBtagDiscr, inline method did not work.
