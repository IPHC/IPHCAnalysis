...
// Inputs: Number of points && range of plots in x,y 
 ...
 //for zut
  ...
  Ninit = 57.0; // Nombre d'events signal MC initial =  MVABDT__FCNCkut57.Integral()
 }
 else{
   //for zct
...
// @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
 ...
 ifstream in_obs;
 if(isZut) in_obs.open("ResultsZut/cls_limits_observed.txt");
 else      in_obs.open("ResultsZct/cls_limits_observed.txt");
 ...
 ifstream in_exp;
 if(isZut) in_exp.open("ResultsZut/cls_limits_expected.txt");
 else      in_exp.open("ResultsZct/cls_limits_expected.txt");
